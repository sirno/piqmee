...
public class CategoriesLogger extends BEASTObject implements Loggable, Function {
    ...
    final public Input<GenericTreeLikelihood> likelihoodInput = new Input<>("treeLikelihood",
    ...
    final public Input<BranchRateModel> branchRateModelInput = new Input<>("branchratemodel",
            "model that provides rates for a tree", Input.Validate.XOR, likelihoodInput);
    ...
    public void initAndValidate() {
        ...
        for (int i = 0; i < tree.getNodeCount() + tree.getLeafNodeCount(); i++) {
            if (i < tree.getLeafNodeCount()
                    && ((QuasiSpeciesNode) tree.getNode(i)).getAttachmentTimesList().length == 1) {
                categories[i] = 0;
            } else if (i < tree.getNodeCount() && tree.getNode(i).isRoot()) {
                categories[i] = 0;
            } else if (i >= tree.getLeafNodeCount() && i < tree.getNodeCount()
                    && ((QuasiSpeciesNode) tree.getNode(i)).getContinuingHaploName() != -1) {
                categories[i] = ((QuasiSpeciesUCRelaxedClockModel) branchRateModel)
                        .getCategories(((QuasiSpeciesNode) tree.getNode(i)).getContinuingHaploName());
            } else {
