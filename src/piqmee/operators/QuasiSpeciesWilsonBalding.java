...
// + "starting point between the first attachment time and the next MRCA with
// another haplotype")
public class QuasiSpeciesWilsonBalding extends QuasiSpeciesTreeOperator {
    ...
    public double proposal() {
        ...
        // count number of pairs valid for WB (may not be the same back and forth
        // without root moves)
        ...
        // double newTime = t_destNode+Randomizer.nextExponential(1.0 / (alpha *
        // t_destNode));
        ...
        // double haploStartMax = getMaxPossibleHaploAttachTime((QuasiSpeciesNode)
        // srcNode, srcHaplo);
        //
        // // Choose attachment point for the moved haplotype - may not necessarily be
        // the origin height
        // ArrayList haploStartMaxNewArray =
        // getMaxPossibleHaploAttachTime((QuasiSpeciesNode) destNode, srcHaplo,
        // newTime);
        // double haploStartMaxNew = (double) haploStartMaxNewArray.get(1);
        //
        // double logHastingsRatioContribution = scaleThisHaplo(node, haploStartMaxNew,
        // haploStartMin, 0,
        ...
        // QuasiSpeciesNode correctTreeFromThisNode1 =
        // disconnectBranch((QuasiSpeciesNode) srcNode, srcHaplo);
        // QuasiSpeciesNode correctTreeFromThisNode2 =
        // connectBranchToRoot((QuasiSpeciesNode) srcNode,
        // (QuasiSpeciesNode) destNode, newTime, srcHaplo,
        // node.getAttachmentTimesList()[0]);
        // qsTree.setRoot((QuasiSpeciesNode) srcNodeP);
        //
        // // Recalculate continuingHaplo and HaploAbove arrays
        // recalculateParentHaploAndCorrectContinuingHaploName(-1, (QuasiSpeciesNode)
        // srcNodeP);
        // // in any case (changed or not the aboveNodeHaplo/parentHaplo array)
        ...
        // // P(moving
        // forth)=(1/(alpha*t_destNode)*exp(-1/(alpha*t_destNode)*(newTime-t_destNode)))
        // // exponentially distr
        // //
        // HR=P(back)/P(forth)=(alpha*t_destNode)*exp(1/(alpha*t_destNode)*(newTime-t_destNode))/(time
        // span moving back)
        // //
        // log(HR)=log(alpha*t_destNode)+(1/(alpha*t_destNode)*(newTime-t_destNode))-log(time
        // span moving back)
        // logHastingsRatio += Math.log(alpha * t_destNode) + (1.0 / alpha) * (newTime /
        // t_destNode - 1.0)
        // - Math.log(t_srcNodeG - Math.max(t_srcNode, t_srcNodeS));
        // //logHastingsRatio += Math.log(origin.getValue()-t_destNode)
        // // - Math.log(t_srcNodeG - Math.max(t_srcNode, t_srcNodeS));
        //
        // //if ((origin.getValue() - Math.max(t_srcNode, t_destNode)) == 0 ||
        // (t_srcNodeG - Math.max(t_srcNode, t_srcNodeS)) == 0) {
        ...
        // // return Double.NEGATIVE_INFINITY;
        // throw new IllegalStateException("problem in hereeeeee:
        // QuasiSpeciesWilsonBalding - some branch lengths are 0?");
        // }
        //
        ...
        // // Implement tree changes:
        // QuasiSpeciesNode correctTreeFromThisNode1 =
        // disconnectBranchFromRoot((QuasiSpeciesNode) srcNode, srcHaplo);
        // QuasiSpeciesNode correctTreeFromThisNode2 = connectBranch((QuasiSpeciesNode)
        // srcNode,
        // (QuasiSpeciesNode) destNode, newTime, srcHaplo,
        // node.getAttachmentTimesList()[0]);
        ...
        // qsTree.countAndSetPossibleStartBranches();
        //
        // // Incorporate probability of choosing current haplotype to move --- only
        // with Felsenstein
        // logHastingsRatio += Math.log(possibleHaplo.size());
        //
        // // Incorporate probability of choosing current haplotype to move back
        ...
        // // Return HR:
        // // P(moving
        // back)=(1/(alpha*t_srcNodeS)*exp(-1/(alpha*t_srcNodeS)*(oldTime-srcNodeS))) //
        // exponentially distr
        // // P(moving forth)=1/(time span moving forth)
        // // HR=P(back)/P(forth)=(time span moving
        // forth)/(alpha*t_srcNodeS)*exp(1/(alpha*t_srcNodeS)*(oldTime-t_srcNodeS))
        // // log(HR)=log(time span moving
        // forth)-log(alpha*t_srcNodeS)-(1/(alpha*t_srcNodeS)*(oldTime-t_srcNodeS))
        // logHastingsRatio += Math.log(t_destNodeP - Math.max(t_srcNode, t_destNode))
        // - Math.log(alpha * t_srcNodeS) - (1.0 / alpha) * (oldTime / t_srcNodeS -
        // 1.0);
        // //logHastingsRatio += Math.log(t_destNodeP - Math.max(t_srcNode, t_destNode))
        // // - Math.log(origin.getValue() - t_srcNodeS);
        // //if ((t_destNodeP - Math.max(t_srcNode, t_destNode)) == 0 ||
        // (origin.getValue() - t_srcNodeS) == 0) {
        // if ((t_destNodeP - Math.max(t_srcNode, t_destNode)) == 0) {
        // // This happens when some branch lengths are zero.
        // // If oldRange = 0, hastingsRatio == Double.POSITIVE_INFINITY and
        // // node i can be catapulted anywhere in the tree, resulting in
        ...
        if (node.getAttachmentTimesList().length > 1) {
            ...
            // Choose attachment point for the moved haplotype
            ArrayList haploStartMaxNewArray = getMaxPossibleHaploAttachTime((QuasiSpeciesNode) destNode, srcHaplo,
                    newTime);
            double haploStartMaxNew = (double) haploStartMaxNewArray.get(1);
        ...
        // recalculateParentHaploAndCorrectContinuingHaploName(-1,
        // correctTreeFromThisNode1);
        // }
        // else if (correctTreeFromThisNode2 == qsTree.getRoot()){
        // recalculateParentHaploAndCorrectContinuingHaploName(-1,
        // correctTreeFromThisNode2);
        // }
        // else {
        // // for source node, need to find what the parent haplo was and start at the
        // changed node
        // if (correctTreeFromThisNode1!=null)
        //// recalculateParentHaploAndCorrectContinuingHaploName(oldParentHaplo[srcHaplo],
        // correctTreeFromThisNode1);
        // recalculateParentHaploAndCorrectContinuingHaploName(qsTree.getParentHaplo()[srcHaplo],
        // correctTreeFromThisNode1);
        // // for the attached haplotype, need to find out new parent haplo and start at
        // the changed node
        // // changed node can be newly attached parent or any node above
        // // getMaxPossibleHaploAttachTime function returned array of Node, time, haplo
        // (above in the tree)
        // if (correctTreeFromThisNode2.getHaploAboveName()!=-1){
        // if ((int) haploStartMaxNewArray.get(2) !=
        // correctTreeFromThisNode2.getHaploAboveName())
        // recalculateParentHaploAndCorrectContinuingHaploName((int)
        // haploStartMaxNewArray.get(2), correctTreeFromThisNode2);
        // else
        //// recalculateParentHaploAndCorrectContinuingHaploName(oldParentHaplo[(int)
        // haploStartMaxNewArray.get(2)], correctTreeFromThisNode2);
        // recalculateParentHaploAndCorrectContinuingHaploName(qsTree.getParentHaplo()[(int)
        // haploStartMaxNewArray.get(2)], correctTreeFromThisNode2);
        // }
        // // if the continuingHaplo is -1, we must have attached the haplo below the
        // moved parent
        // // there can however still be some other haplo up in the tree (and not coming
        // to this part)
        // else{
        ...
        // recalculateParentHaploAndCorrectContinuingHaploName(contHaplo,
        // (QuasiSpeciesNode) srcNode);
        // }
        // }
        // in any case (changed or not the aboveNodeHaplo/parentHaplo array) recalculate
        // countPossibleStartBranches
        ...
        if ((t_destNodeP - Math.max(t_srcNode, t_destNode)) == 0
            ...
            throw new IllegalStateException(
                    "problem in hereeeeee: QuasiSpeciesWilsonBalding - some branch lengths are 0?");
        }
    ...
    private boolean invalidSrcNode(Node srcNode) {
        ...
        if (parent.isRoot()) {
            ...
            // if the parent is root then the only possibility is the sister branch
            // tip/node,
    ...
    /**
     * Returns true if destNode CANNOT be used for the Wilson-Balding move in
     * conjunction
     * with srcNode.
     *
     * @param srcNode
    ...
    /**
     * Function to count how many valid source and destination node pairs there are
     * for wilson-balding operator
     *
     */
